...
*/
    ...
    class Visitor<FunctorType, std::tuple<Visitables...>, V> : public VisitorBase<Visitables...> {
    public:
        ...
        void cvisit(const V &o) const final { func(o); }

        void visit(V &o) const final { func(o); }
    ...
    public:
        Visitor(FunctorType &&f)
                : Visitor<FunctorType, std::tuple<Visitables...>, V...>{std::forward<FunctorType>(f)} {
        }

        template<typename... Types>
        Visitor(Types &&... Args)
                : Visitor<FunctorType, std::tuple<Visitables...>, V...>{std::forward<Types>(Args)...} {
        }
        ...
        void cvisit(const VC &o) const final { func(o); }

        void visit(VC &o) const final { func(o); }
    };
...
// Factory function for the Visitor class, effectively constitutes its interface
    ...
    std::unique_ptr<VisitorBase<Types...>> make_visitor(V &&v_obj) {
        return std::make_unique<Visitor<V, std::tuple<Types...>, Types...>>(std::forward<V>(v_obj));
    }
