...
void Factory< KeyType, ProductBase >::registerCreator(const KeyType& key)
{
    static_assert(std::is_base_of< ProductBase, Product >::value,
                  "The product type you are trying to register must be derived from the declared "
                  "base class\n");
    ...
    if (it != getCreatorMap().end())
        throw(std::invalid_argument(
            "The creator you are trying to register is already registered\n"));

    getCreatorMap().insert(
        std::make_pair(key, std::make_shared< Creator< KeyType, ProductBase, Product > >()));
}

template < typename KeyType, typename ProductBase >
void Factory< KeyType, ProductBase >::unregisterCreator(const KeyType& key)
{
    auto it = getCreatorMap().find(key);

    if (it == getCreatorMap().end())
        throw(std::invalid_argument(
            "The creator you are trying to unregister was never registered.\n"));

    getCreatorMap().erase(key);
}

template < typename KeyType, typename ProductBase >
typename Factory< KeyType, ProductBase >::s_ptr_t
Factory< KeyType, ProductBase >::getCreator(const KeyType& key)
{
    auto it = getCreatorMap().find(key);

    if (it == getCreatorMap().end())
        throw(
            std::invalid_argument("The creator you are trying to access was never registered.\n"));
