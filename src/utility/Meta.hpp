...
struct array_to_valseq
{
    ...
    using type = typename convert_to_val_seq< ArrayClass,
...
{
    using type =
        typename add_to_valseq < I, typename rep_val < I, N - 1 >::type >::type;
};
...
{
    using type =
        typename cat_valseq< typename rep_val<Vals, N>::type ... >::type;
};
...
{
    using type =
        typename cat_valseq < typename repeat_valseq < T, N - 1 >::type, T >::type;
};
...
{
    using type = typename repeat_valseq< value_sequence<T, Vals ...>,
          sizeof...(Vals) >::type;
};
...
{
    using type =
        M< C< get_valseq_n<I1, Ints>::value, get_valseq_n<I2, Ints>::value > ... >;
};
