...
auto nodePhysicalLocation(const Element< T, O >& element)
{
    ...
    std::ranges::transform(
        ref_locs, begin(retval), [&](const auto& xi) { return map::mapToPhysicalSpace(element.getData(), xi); });
    return retval;
}
...
{
    return map::mapToPhysicalSpace(element.getData(), getNodeLocations< T, O >()[i]);
}
} // namespace lstr::mesh
