...
auto makeLineMesh(Rx&& distx, d_id_t domain_id = 0) -> MeshPartition< 1 >
    requires std::convertible_to< std::ranges::range_value_t< std::decay_t< Rx > >, val_t >
{
    ...
    domains[domain_id].elements.getVector< Element< ElementType::Line, 1 > >().reserve(e_dx);
    ...
    for (el_id_t id : std::views::iota(0u, e_dx))
    {
        ...
        emplaceInDomain< ElementType::Line, 1 >(domains[domain_id], nodes, verts, id);
    }
