...
template < el_o_t O, el_locind_t I >
val_t evaluateLagrangeLineBasisFun(const mesh::Point< 1 >& point)
{
    return getLagrangeLineBasisPolynomial< O, I >().evaluate(point.x());
}

template < el_o_t O, el_locind_t I >
val_t evaluateLagrangeLineBasisFunDer(const mesh::Point< 1 >& point)
{
...
val_t evaluateLineBasisFun(const mesh::Point< 1 >& point)
{
...
template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateLineBasisFunDer(const mesh::Point< 1 >& point)
{
    if constexpr (BT == BasisType::Lagrange)
...
template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateQuadBasisFun(const mesh::Point< 2 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto xi_ind         = I % nodes_per_edge;
    constexpr auto eta_ind        = I / nodes_per_edge;

    return evaluateLineBasisFun< O, xi_ind, BT >(mesh::Point{point.x()}) *
           evaluateLineBasisFun< O, eta_ind, BT >(mesh::Point{point.y()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateQuadBasisFunDerXi(const mesh::Point< 2 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto xi_ind         = I % nodes_per_edge;
    constexpr auto eta_ind        = I / nodes_per_edge;

    return evaluateLineBasisFunDer< O, xi_ind, BT >(mesh::Point{point.x()}) *
           evaluateLineBasisFun< O, eta_ind, BT >(mesh::Point{point.y()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateQuadBasisFunDerEta(const mesh::Point< 2 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto xi_ind         = I % nodes_per_edge;
    constexpr auto eta_ind        = I / nodes_per_edge;
    ...
    return evaluateLineBasisFun< O, xi_ind, BT >(mesh::Point{point.x()}) *
           evaluateLineBasisFunDer< O, eta_ind, BT >(mesh::Point{point.y()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateHexBasisFun(const mesh::Point< 3 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    ...
    return evaluateQuadBasisFun< O, xi_eta_ind, BT >(mesh::Point{point.x(), point.y()}) *
           evaluateLineBasisFun< O, zeta_ind, BT >(mesh::Point{point.z()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateHexBasisFunDerXi(const mesh::Point< 3 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto nodes_per_face = nodes_per_edge * nodes_per_edge;
    constexpr auto xi_eta_ind     = I % nodes_per_face;
    ...
    return evaluateQuadBasisFunDerXi< O, xi_eta_ind, BT >(mesh::Point{point.x(), point.y()}) *
           evaluateLineBasisFun< O, zeta_ind, BT >(mesh::Point{point.z()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateHexBasisFunDerEta(const mesh::Point< 3 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto nodes_per_face = nodes_per_edge * nodes_per_edge;
    constexpr auto xi_eta_ind     = I % nodes_per_face;
    ...
    return evaluateQuadBasisFunDerEta< O, xi_eta_ind, BT >(mesh::Point{point.x(), point.y()}) *
           evaluateLineBasisFun< O, zeta_ind, BT >(mesh::Point{point.z()});
}

template < el_o_t O, el_locind_t I, BasisType BT >
val_t evaluateHexBasisFunDerZeta(const mesh::Point< 3 >& point)
{
    constexpr auto nodes_per_edge = O + 1;
    constexpr auto nodes_per_face = nodes_per_edge * nodes_per_edge;
    constexpr auto xi_eta_ind     = I % nodes_per_face;
    ...
    return evaluateQuadBasisFun< O, xi_eta_ind, BT >(mesh::Point{point.x(), point.y()}) *
           evaluateLineBasisFunDer< O, zeta_ind, BT >(mesh::Point{point.z()});
}
} // namespace detail

template < mesh::ElementType T, el_o_t O, el_locind_t I, BasisType BT, DerDim DER_DIM = DerDim::NoDer >
    requires(I < mesh::Element< T, O >::n_nodes and static_cast< dim_t >(DER_DIM) <= mesh::Element< T, O >::native_dim)
struct ReferenceBasisFunction
{
    val_t operator()(const mesh::Point< mesh::Element< T, O >::native_dim >& point) const
    ...
    {
        if constexpr (T == mesh::ElementType::Quad)
    ...
    {
        if constexpr (T == mesh::ElementType::Hex)
            return detail::evaluateHexBasisFunDerZeta< O, I, BT >(point);
    }
};

template < mesh::ElementType T, el_o_t O, BasisType BT >
auto computeRefBasis(const mesh::Point< mesh::Element< T, O >::native_dim >& point)
{
...
template < mesh::ElementType T, el_o_t O, BasisType BT >
auto computeRefBasisDers(const mesh::Point< mesh::Element< T, O >::native_dim >& point)
{
