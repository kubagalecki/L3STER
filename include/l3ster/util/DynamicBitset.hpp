...
public:
    ...
    [[nodiscard]] bool atomicTest(std::size_t pos, std::memory_order order = std::memory_order_seq_cst) noexcept
    {
        const auto [ull_ind, bit_pos] = posInds(pos);
        std::atomic_ref atomic_val{m_data[ull_ind].value};
        return atomic_val.load(order) & getMask(bit_pos);
    }
    void atomicSet(std::size_t pos, std::memory_order order = std::memory_order_seq_cst) noexcept
    {
        const auto [ull_ind, bit_pos] = posInds(pos);
        std::atomic_ref atomic_val{m_data[ull_ind].value};
        atomic_val.fetch_or(getMask(bit_pos), order);
    }
    void atomicReset(std::size_t pos, std::memory_order order = std::memory_order_seq_cst) noexcept
    {
        const auto [ull_ind, bit_pos] = posInds(pos);
        std::atomic_ref atomic_val{m_data[ull_ind].value};
        atomic_val.fetch_and(~getMask(bit_pos), order);
    }
    void atomicFlip(std::size_t pos, std::memory_order order = std::memory_order_seq_cst) noexcept
    {
        const auto [ull_ind, bit_pos] = posInds(pos);
        std::atomic_ref atomic_val{m_data[ull_ind].value};
    ...
    public:
        ...
        operator bool() { return m_target->test(m_pos); }
