...
auto computePhysBasisDersAtQpoints(
    const std::array< Eigen::Matrix< val_t, QL, n_bases >, size_t(native_dim) >& ref_ders_at_qp,
    const std::array< detail::jacobian_t< native_dim >, size_t(QL) >&            jacobians_at_qp)
{
    std::array< Eigen::Matrix< val_t, QL, n_bases >, native_dim > retval;
    for (size_t qp_ind = 0; qp_ind < QL; ++qp_ind)
    {
        const Eigen::Matrix< val_t, native_dim, n_bases > ref_ders = [&] {
            Eigen::Matrix< val_t, native_dim, n_bases > ders_at_qp;
        ...
        const Eigen::Matrix< val_t, native_dim, n_bases > phys_ders =
            computePhysBasisDers(jacobians_at_qp[qp_ind], ref_ders);
        for (size_t bas_ind = 0; bas_ind < n_bases; ++bas_ind)
            for (int dim_ind = 0; dim_ind < native_dim; ++dim_ind)
