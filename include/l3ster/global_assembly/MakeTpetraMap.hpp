...
std::vector< global_dof_t > getNodeDofs(const std::vector< n_id_t >&                      nodes,
                                        const detail::node_interval_vector_t< n_fields >& dof_intervals)
{
    std::vector< global_dof_t > retval;
    retval.reserve(n_fields * nodes.size());
...
template < size_t n_fields >
Teuchos::RCP< Tpetra::Map< local_dof_t, global_dof_t > >
              ...
              const MpiComm&                                    comm)
{
    ...
    auto       teuchos_comm = Teuchos::rcp(new const Teuchos::MpiComm< int >{comm.get()}); // NOLINT
    return Teuchos::rcp(
        new map_t{Teuchos::OrdinalTraits< Tpetra::global_size_t >::invalid(), dofs, 0, teuchos_comm}); // NOLINT
}
} // namespace lstr
