...
HwlocWrapper::~HwlocWrapper()
{
    for (auto& bitmap_vector : mask_map)
        std::ranges::for_each(bitmap_vector, [](hwloc_cpuset_t cpu) { hwloc_bitmap_free(cpu); });
    hwloc_topology_destroy(topo);
}

void HwlocWrapper::bindThread(size_t node, size_t cpu) const
{
    if (hwloc_set_cpubind(topo, mask_map[node][cpu], HWLOC_CPUBIND_THREAD))
        throw std::runtime_error{"could not bind thread to core"};
}

std::pair< size_t, size_t > HwlocWrapper::getLastThreadLocation() const
{
...
void HwlocWrapper::populateMaskMap() noexcept
{
    ...
    detail::hwlocBitmapForEachWrapper(node_set, [&](hwloc_const_nodeset_t node) {
        ...
        mask_map.push_back(std::move(cpus));
    });
}

void HwlocWrapper::groupCpus()
{
                                                          ...
                                                          HwlocWrapper::findCpuIf(const F& fun) const noexcept
{
    ...
    const size_t                node_dist =
        std::distance(mask_map.begin(), std::ranges::find_if(mask_map, [&](const std::vector< hwloc_cpuset_t >& cpus) {
                          ret.second = std::distance(cpus.begin(), std::ranges::find_if(cpus, fun));
                          return ret.second < cpus.size();
                      }));
    ret.first = node_dist == mask_map.size() ? std::numeric_limits< size_t >::max() : node_dist;
    return ret;
}
} // namespace lstr
